...
The admin-only page at `/admin/backups` can create backups, upload a previously created `.tar.gz` backup archive, list recent backup manifests, download a listed backup, and restore from a listed backup. Browser restore is destructive and requires typing `RESTORE` plus the exact backup filename. Downloads are served only through admin-mode-protected routes; no public download links are exposed.
