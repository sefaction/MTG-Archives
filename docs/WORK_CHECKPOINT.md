...
- Current recovery changes pin client 16, reject mismatched create/restore, preflight archive/payload/targets and use one database transaction for schema replacement+SQL load. Added missing/corrupt dump and real SQL check-constraint rollback fixtures. Source pricing cache is count-only, authoritative tables full-content hashed. Typecheck and 21 focused tests passed before the latest rollback fixture. Next: commit/rebuild corrected client image, run a FRESH capture/drill (old-client archive should not be reused as success evidence), full verification and PR. No recovery PR open yet; no new merge approval.
