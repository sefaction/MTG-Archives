...
Latest Pricing continuation on `feat/pricing-summaries` (stacked on `fix/pricing-responsive-queries`): compact scope and monthly summaries, daily deduplicated points, an idempotent touched-key refresher and a resumable full rebuild command are implemented but not yet committed, pushed or loaded into the local web image. The local pricing database was backfilled from 1,971,376 raw rows: 5,734 printings in 217,147 ms. An isolated fixture passed repeated rebuild and corrected-observation checks. The summary sizes after backfill were about 22 MB scope, 747 MB daily including indexes and 63 MB monthly; all raw observations remain. The daily size demonstrates why #330's verified retention/compaction gate still matters. Latest dashboard-function warm samples for 4,146 owned printings: Collection about 2–7 seconds (daily trend dominates), Market about 1–2 seconds with one 60-row ranked query, Data about 1 second; all now return available. All-history monthly Collection took about 1.9 seconds. These are not p50/p95 browser budgets. Source changes include a summary readiness/freshness check and monthly chart labeling; Docker still has the earlier image until rebuilt. 590 unit tests and typecheck pass on current uncommitted source. Next: build a cumulative image with Public PR #338 plus Pricing source, browser/route performance tests, sample raw-versus-summary checks, then review PRs. No merges or issue closures.
