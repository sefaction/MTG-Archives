...
import { SampleHands } from "@/components/SampleHands";
...
export default async function DeckSampleHandsPage({
  params,
}: {
      ...
      <section className="app-panel p-4">
        ...
        <div className="mt-1 flex flex-wrap items-end justify-between gap-2">
          ...
          <p className="max-w-xl text-xs text-[var(--app-muted)]">
            Shuffle the mainboard, test opening hands, and walk through London
            mulligans without changing the saved deck. Sideboard and maybeboard
            cards are excluded.
          </p>
        </div>
      </section>

      <DeckToolsNav deckId={deck.id} active="hands" />
      <SampleHands cards={deck.cards} initialSeed={randomUUID()} />
    </main>
  );
}
